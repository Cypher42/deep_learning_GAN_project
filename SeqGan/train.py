...
def train_epoch(sess, trainable_model, num_iter,
    ...
    with open('results.txt','a+') as fp:
        try:
            l1 = [words[x] if words else x for x in supervised_gen_x] if supervised_gen_x is not None else None
            l2 = [words[x] if words else x for x in unsupervised_gen_x] if unsupervised_gen_x is not None else None
            if l1 is None:
                pass
            else:
                fp.write(l1)
