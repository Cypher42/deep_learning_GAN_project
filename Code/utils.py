...
def get_full_clips(data_dir, num_clips, num_rec_out=1):
    ...
    for clip_num, ep_dir in enumerate(ep_dirs):
        # MY ADDITION START
        clip_length = 0
        while clip_length < c.HIST_LEN + num_rec_out:
            clip_dir = np.random.choice(glob(os.path.join(ep_dir, '*')))
            ep_frame_paths = sorted(glob(os.path.join(clip_dir, '*')))
            clip_length = len(ep_frame_paths)
        # MY ADDITION END
        ...
        for frame_num, frame_path in enumerate(clip_frame_paths):
            ...
            # MY ADDITION START
            for i in range(len(norm_frame)):
                for j in range(len(norm_frame[i])):
                    clips[clip_num, i, j, frame_num * 3:(frame_num + 1) * 3] = norm_frame[i, j]
            # MY ADDITION END
